...
$ theta(bold(x)) = bold(beta)^Tr bold(x) = M bold(x) $ $
  bold(theta) = X bold(beta)
$
2. Link function application:
$ psi colon quad theta = psi(mu) $ $
  psi(Ex[y|bold(x)]) = bold(beta)^Tr bold(x)
$ $
  psi(Ex[y|X])=
  X bold(beta)
$
...
Where:
$ pdv(A, theta) = sigma(theta) = 1/(1 + e^(-theta)) $ $
  psi(mu) = sigma^(-1)(p) = ln
  p/(1-p) = "logit" p
$
...
$Y$ follows a Bernoulli distribution: $ y_i tilde cal(B)(p), quad p := Pr[y_i = 1] $
Here,
$p$ represents the success probability in a single trial. The canonical form of the
Bernoulli distribution is: $ y_i tilde f(y|theta) = sigma(-theta) dot e^(theta dot y), quad sigma(theta) = 1 / (1 +
e^(-theta)) $
...
For the Bernoulli distribution, the link function takes the form: $ psi(mu) = log mu/(1-mu) = "logit" mu $
#columns(
  2,
)[
  == Cross-entropy Loss
  ...
  $
    -l(theta) & = -sum_i {theta dot y_i + log e^(-theta) / (1+e^(-theta))} \
              ...
              & = sum_i cases(
                  log(1+e^(-theta)) \, "if" y = 1, log(1+e^(theta)) \, "if" y
                  = 0
                ) \
