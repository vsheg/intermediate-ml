...
+ All parameters $bold(theta) :> bold(beta)$ are independent and linear, so the joint
  ...
  #note[These distributions impose prior constraints on the model coefficients, effectively
...
=== Applying MAP
#note[Here we denote pdf underscripts with letters corresponding to random variables]
