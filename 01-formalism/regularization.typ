...
+ The posterior distribution of responses $y$ depends on parameters $bold(theta)$ and
  ...
  #note[Support of a random variable $Y$ is the set of all possible values $y^*_1, y^*_2, ...$ that $Y$ can
...
=== Model
Here we make a specific choice of the prior distribution of parameters $f_Theta (bold(theta)|bold(gamma))$ and
the data:
...
+ All parameters $bold(theta) :> bold(beta)$ are independent and linear, so the joint
  ...
  #note[These distribution impose prior constraints on the model coefficients, effectively
    reducing the solution space]
  $ f_Theta (bold(theta) = bold(beta)|bold(gamma)) = product_(j=1)^k f_Theta (beta_j|bold(gamma)) $
...
+ Each parameter $beta_j$ follows a Gaussian distribution with two hyperparameters common
  ...
  $ f (beta_j|mu, tau) = 1/(sqrt(2 pi) tau) e^(-beta_j^2 \/ (2 tau^2)) ~ N(mu, tau). $
+ The data is generated by a linear model with Gaussian noise $N(0, sigma)$:
  #note[Both errors $bold(epsilon) ~ N(bold(0), sigma I)$ and model's parameters $beta ~ N(bold(0), tau I)$ follow
    multivariate Gaussian distributions with zero mean and different covariance matrices $sigma I$ and $tau I$ respectively.]
  #note[Dimensions of vectors $bold(epsilon)$ and
...
=== Assumptions
We apply the maximum a posteriori (MAP) method to incorporate prior knowledge about
parameter distributions: $
  ...
  -> max_bold(beta)
$
Let's write the error distribution through the error distribution:
$
